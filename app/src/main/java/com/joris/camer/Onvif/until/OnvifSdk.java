...
import com.joris.camer.Onvif.onvif.FindDevicesThread;
import com.joris.camer.Onvif.onvif.GeNetWorkInterfaceThread;
import com.joris.camer.Onvif.onvif.GetDeviceInfoThread;
import com.joris.camer.Onvif.onvif.GetImagingSettingsThread;
import com.joris.camer.Onvif.onvif.GetSnapshotInfoThread;
import com.joris.camer.Onvif.onvif.SetImagingSettingsThread;
import com.joris.camer.Onvif.onvif.SetNetworkInterfaceThread;
import com.joris.camer.Onvif.onvif.SetSystemDateAndTimeThread;
import com.joris.camer.Onvif.onvif.SetUserThread;
import com.joris.camer.Onvif.onvif.StartFirmwareUpgradeThread;
import com.joris.camer.Onvif.onvif.SystemRebootThread;
...
public class OnvifSdk {
    ...
    public static String MONITOR_CAMERA_PSD = "admin";

    public static HashMap<String, Device> deviceHashMap = new HashMap<>();

    private static Context mContext;
    public static boolean searching; //正在搜索
    ...
    public synchronized static void findDevice(Context context){
        ...
        searching = true;
        deviceHashMap.clear();
        findDevice(context, "192.168.43.255", new FindDevicesThread.FindDevicesListener() {
            @Override
                ...
                if (devices != null) {
                    for (Device device : devices) {
                        if (device.getServiceUrl() != null && device.getIpAddress() != null) {
                            ...
                            getDeviceInfo(mContext, device, new GetDeviceInfoThread.GetDeviceInfoCallBack() {
                                ...
                                public void getDeviceInfoResult(boolean isSuccess, Device device, String errorMsg) {
                                    if (isSuccess) {
                                        ...
                                        LogClientUtils.d(tag, "发现摄像头设备" + JSONObject.wrap(device));
                                    } else {
                            ...
                            deviceHashMap.put(device.getIpAddress(), device);
                        }
                    }
